"""Twitter/X Lead Scraper - Crawlee + high concurrency (100+ leads in <1 min).
...
def calc_quality(text: str) -> float:
    text = text.lower()
    score = 20
    if any(k in text for k in ["hiring", "looking for", "job"]):
        score += 25
    if any(k in text for k in ["developer", "engineer", "dev"]):
        score += 15
...
def _extract_ddg_results(soup: Any) -> list[dict[str, str]]:
...
def _extract_bing_results(soup: Any) -> list[dict[str, str]]:
    results: list[dict[str, str]] = []
    for li in soup.select("li.b_algo"):
        ...
        snippet = snippet_el.get_text(" ", strip=True) if snippet_el else ""
        if href and title and ("twitter.com" in href or "x.com" in href):
            results.append({"url": href, "title": title, "snippet": snippet})
    return results
...
async def main() -> None:
    async with Actor:
        ...
        Actor.log.info(
            f"🚀 Twitter/X Scraper (Crawlee) | Keywords: {len(keywords)}, maxResults: {max_results}, target 100+ in <1 min"
        )

        seen: set[str] = set()
        leads_count = 0

        # Build all SERP URLs: DDG (site:twitter + site:x) + Bing fallback per keyword
        start_requests: list[Request] = []
        for kw in keywords:
            for domain in ("twitter.com", "x.com"):
                start_requests.append(
                    Request.from_url(
                        f"https://duckduckgo.com/html/?q=site:{domain}+{quote_plus(kw)}",
                        user_data={"engine": "ddg", "keyword": kw},
                    )
                )
            start_requests.append(
                Request.from_url(
                    f"https://www.bing.com/search?q=site:twitter.com+{quote_plus(kw)}",
        ...
        async def handle_serp(context: BeautifulSoupCrawlingContext) -> None:
            ...
            for item in results:
                ...
                if not link_url or link_url in seen:
                    continue
                if "twitter.com" not in link_url and "x.com" not in link_url:
                    continue
                seen.add(link_url)
