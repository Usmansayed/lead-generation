"""Facebook Lead Scraper - Crawlee + high concurrency (100+ leads in <1 min).
...
def _extract_bing_results(soup) -> list[dict]:
    ...
    for li in soup.select("li.b_algo"):
        ...
        snippet = snippet_el.get_text(" ", strip=True) if snippet_el else ""
        if href and title and "facebook.com" in href:
            results.append({"url": href, "title": title, "snippet": snippet})
    return results


def _is_facebook_post(url: str) -> bool:
    if "facebook.com" not in url or "facebook.com/login" in url or "facebook.com/recover" in url:
        return False
    return any(p in url for p in ["/posts/", "/permalink.php", "/groups/"])
...
async def main() -> None:
    async with Actor:
        ...
        Actor.log.info(
            f"🚀 Facebook Scraper (Crawlee) | Keywords: {len(keywords)}, maxResults: {max_results}, target 100+ in <1 min"
        )
        ...
        for kw in keywords:
            start_requests.append(
                Request.from_url(
                    f"https://duckduckgo.com/html/?q=site:facebook.com+{quote_plus(kw)}",
                    user_data={"engine": "ddg", "keyword": kw},
                )
            )
            start_requests.append(
                Request.from_url(
                    f"https://www.bing.com/search?q=site:facebook.com+{quote_plus(kw)}",
        ...
        async def handle_serp(context: BeautifulSoupCrawlingContext) -> None:
            ...
            for item in results:
                ...
                if not link_url or link_url in seen:
                    continue
                if not _is_facebook_post(link_url):
                    continue
                seen.add(link_url)
